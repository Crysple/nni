...
class GeneralController():
  ...
  def build_trainer(self):
    ...
    #self.tf_variables = sorted(self.tf_variables, key=lambda var:var.name)
    self.grad_placeholder = [(tf.placeholder("float", shape=grad[i].get_shape()), self.tf_variables[i]) for i in len(self.tf_variables)]
    ...
    train_op = opt.apply_gradients(zip(grads, self.grad_placeholder), global_step=self.train_step)
    self.train_op, self.lr, self.optimizer = train_op, learning_rate, opt
